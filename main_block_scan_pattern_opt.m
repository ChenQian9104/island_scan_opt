...
for iterNum = 1:150
    ...
    for i = 1:nstep
        ...
        elseif i == nstep  % corresponding to step n+1: the last step for cutting off
            ...
            f_act = ( K{i} - K{i-1} )*U; % activation force vector 
            for ele = (i-1)*nelx*nely + 1:i*nelx*nely
                ele_epsilon_act(:,ele) = Be*U(edofMat1(ele,:)');
                f_th = thermal_load(Be,C,epsilon(:,ele),dx,dy,dz );
                F_th(edofMat1(ele,:)',1) = F_th(edofMat1(ele,:)',1) + f_th(:);
            end      
            F_th = F_th + f_act; % Two parts: 1. inherent strain vector; 2. Activation force vector 
            U(freedofs,:) = K{i}(freedofs,freedofs)\F_th(freedofs,:);
        end      
    end  
    if iterNum == 1
    ...
    object = [object U(3*target_node,1)]; 
    ...
    theta = mod(theta,pi); % ouput scan orientation: [0, pi)
